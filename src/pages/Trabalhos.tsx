...
export default function Trabalhos() {
  const { trabalhos, locais, clientes, dataLoading } = useApp();
    ...
    <div className="space-y-5 animate-slide-in">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">{trabalhos.length} OS cadastradas</p>
        </div>
      ...
      <div className="flex flex-col sm:flex-row gap-2 flex-wrap">
        <div className="relative flex-1 min-w-48">
          ...
          <Input placeholder="Buscar ordens de serviço..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9" />
        </div>
        <Select value={obraFilter} onValueChange={setObraFilter}>
          ...
          <SelectContent>
            ...
            <SelectItem value="Galvanização">⚙️ Galvanização</SelectItem>
      ...
      {dataLoading ? (
        ...
        <div className="bg-card rounded-xl border border-border shadow-card overflow-hidden">
          {filtered.length === 0 ? (
            <p className="text-sm text-muted-foreground p-8 text-center">Nenhuma ordem de serviço encontrada.</p>
          ) : (
